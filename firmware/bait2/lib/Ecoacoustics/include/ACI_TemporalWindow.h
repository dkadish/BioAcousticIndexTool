...
{
public:
    ACI_TemporalWindow(int interval, FFTReader &fft, bool doFilter = false, bool doDiscardAdjacentZeros = false, int debugInterval = 0);
...
private:
    ...
    bool _filter_fft;                        // Should FFT values be set to zero when below a threshold? See Farina et. al. (2016). The Application of ...
    ...
    bool _discard_adjacent_zeros; // Ignore frame differences when one of the two frames is zero. See  Farina et. al. (2016).
